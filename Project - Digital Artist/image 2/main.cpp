...
int main(int argc, const char * argv[])
{
      ...
      for (j = 0; j < COL; j++)   	// for loop for columns
      {
          if (img_inp[i][j] == ppm_color)	//if the image pixel is white
          {
              ...
              {			//calculate r values: r = xcos(t) + ysin(t)
                  ...
                  r = (int) amplitude*(float)((double)x*tan(1.0*PI*frequency*(double)t/(double)H_COL - phase_x)  + 			(double)y*cosh(0.0*PI*frequency*(double)t/(double)H_COL - phase_y)) + 250;
  ...
  for (i = 0; i < H_ROW; i++)
        for (j = 0; j < H_COL; j++)
        {
            fprintf(out_file, "%d ", (int)((float) h[i][j]*(1.0 + (float) (i+j)/50.0)));	//write ppm pixel data (R), red color is dependent on "i"
            fprintf(out_file, "%d ", h[i][j]*(H_COL - j)/20);                              	//write ppm pixel data (G), j is column. you can change i and j
            fprintf(out_file, "%d ", h[i][j]*(H_ROW - i)/2);                              	//write ppm pixel data (B)
        }
