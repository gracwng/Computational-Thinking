...
int main(int argc, const char * argv[])
{
      ...
      for (j = 0; j < COL; j++)   	// for loop for columns
      {
          if (img_inp[i][j] == ppm_color)	//if the image pixel is white
          {
              ...
              {			//calculate r values: r = xcos(t) + ysin(t)
                  ...
                  r = (int) amplitude*(float)((double)x*cos(100.05*PI*frequency*(double)t/(double)H_COL - phase_x)  + 			(double)y*sinh(3.0*PI*frequency*(double)t/(double)H_COL - phase_y)) + 300;
                    /* Creating Circular Hough Space */
 	                  xx = r * sin(3.0*PI*(double)t/(double)H_COL - phase_x);
                    yy = r * cos(4.0*PI*(double)t/(double)H_ROW - phase_y);
  ...
  for (i = 0; i < H_ROW; i++)
        for (j = 0; j < H_COL; j++)
        {
            fprintf(out_file, "%d ", (int)((float) h[i][j]*(5.0 + (float) (i+j)/ 5.0)));	//write ppm pixel data (R), red color is dependent on "i"
            fprintf(out_file, "%d ", h[i][j]*(H_COL - j)/5);                              	//write ppm pixel data (G), j is column. you can change i and j
            fprintf(out_file, "%d ", h[i][j]*(H_ROW - i)/5);                              	//write ppm pixel data (B)
        }
