...
int main(int argc, const char * argv[])
{
    ...
    double amplitude = 5.0; 		//amplitude constant for sine wave
    double frequency = 2.0;		//frequency constant for sine wave
      ...
      for (j = 0; j < COL; j++)   	// for loop for columns
      {
          if (img_inp[i][j] == ppm_color)	//if the image pixel is white
          {
              ...
              {			//calculate r values: r = xcos(t) + ysin(t)
                  ...
                  r = (int) amplitude*(float)((double)x*tgamma(2.0*PI*frequency*(double)t/(double)H_COL - phase_x)  + 			(double)y*lgamma(2.0*PI*frequency*(double)t/(double)H_COL - phase_y));
                    /* Creating Circular Hough Space */
 	                  xx = r * tgamma(2.0*PI*(double)t/(double)H_COL - phase_x);
                    yy = r * lgamma(2.0*PI*(double)t/(double)H_ROW - phase_y);
                    if (xx >= -H_COL/2 && xx < H_COL/2 && yy >= -H_ROW/2 && yy < H_ROW/2)
                        h[xx+H_COL/2][yy+H_ROW/2] = h[xx+H_COL/2][yy+H_ROW/2] + 3;
              }   
          }
      }
  ...
  for (i = 0; i < H_ROW; i++)
        for (j = 0; j < H_COL; j++)
        {
            ...
            fprintf(out_file, "%d ", h[i][j]*(H_COL - j)/20);                              	//write ppm pixel data (G), j is column. you can change i and j
            fprintf(out_file, "%d ", h[i][j]*(H_ROW - i)/30);                              	//write ppm pixel data (B)
        }
